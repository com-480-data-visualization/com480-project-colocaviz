document.addEventListener("DOMContentLoaded", function () {
  const container = document.getElementById("carbon");
  const width = container.clientWidth;
  const height = container.clientHeight;

  const svg = d3.select("#carbon")
    .append("svg")
    .attr("width", width)
    .attr("height", height)
  ...
  d3.json("data/data_carbon.json").then(data => {
  ...
  function drawCustomTreemap(node) {
    ...
    if (node.children && node.children.length > 0) {
      ...
      const color = d3.scaleSequential()
        ...
        .interpolator(d3.interpolateRgb("#D2B48C", "#8B5E3C"));
      ...
      const cell = group.selectAll("g")
        ...
        .on("mouseenter", (event, d) => {
          ...
          highlightCorrespondingCell(d.data.name, "carbon");
        })
        .on("mouseleave", (event, d) => {
          ...
          unhighlightCorrespondingCell("carbon");
        });
      ...
      textGroup.append("text")
        ...
        .attr("dy", d => d.data.has_production ? "-1em" : "-0.5em") // Adjust position if production data available
        .style("opacity", 0)
        .text(d => d.data.name)
      ...
      textGroup.append("text")
        ...
        .attr("text-anchor", "middle")
        .attr("dominant-baseline", "middle")
        .attr("dy", d => d.data.has_production ? "0em" : "0.7em") // Adjust position if production data available
        .style("opacity", 0)
      ...
      leaf.append("rect")
        ...
        .attr("fill", "#8B5E3C"); // Couleur pour les nœuds feuilles
      ...
      textGroup.append("text")
        ...
        .attr("text-anchor", "middle")
        .attr("dominant-baseline", "middle")
        .attr("dy", node.data.has_production ? "-1em" : "-0.5em")
      ...
      textGroup.append("text")
        ...
        .attr("text-anchor", "middle")
        .attr("dominant-baseline", "middle")
        .attr("dy", node.data.has_production ? "0.2em" : "1em")
  ...
  function adjustTextSize(cell) {
    cell.selectAll("g.text-group").each(function(d) {
      ...
      // Add production text height if it exists
      if (productionNode && d.data.has_production) {
        totalHeight += productionNode.getBBox().height;
      }
      
      while ((Math.max(nameWidth, valueWidth, productionWidth) > rectWidth - 6 || totalHeight > availableHeight) && fontSize > 3) {
        fontSize -= 1;
        textGroup.selectAll("text").style("font-size", fontSize + "px");
  ...
  function updateWaterTreemap(path, source) {
    const event = new CustomEvent("updateWater", { 
  ...
  document.addEventListener("updateCarbon", function (e) {
    if (e.detail.source === "carbon") return;
